...
class VideoPlayerObserver: VideoSourceLoaderListener {
  ...
  private var _status: VideoPlayerStatus = .idle
  private var status: VideoPlayerStatus {
    get {
      return _status
    }
    set {
  ...
  init(owner: HybridVideoPlayer, videoSourceLoader: VideoSourceLoader) {
    self.owner = owner
  ...
  func notifyPlayerDeinit(player: HybridVideoPlayer) {
    delegates.forEach { delegate in
  ...
  func startOrUpdateTimeUpdates(forInterval interval: Double) {
    ...
    self.periodicTimeObserver = player?.addPeriodicTimeObserver(forInterval: interval, queue: .main) { [weak self] _ in
      guard let self, let player, let owner = self.owner else {
        return
      }
...
fileprivate extension AVPlayerItem.Status {
  func toVideoPlayerStatus(isPlaybackBufferEmpty: Bool) -> VideoPlayerStatus {
    switch self {
    case .unknown:
      return .loading
    case .failed:
