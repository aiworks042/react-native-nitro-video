...
internal final class CacheVideoAssetTransportProvider: VideoAssetTransportProvider {
  let identifier = "nitro-video.cache"
  let priority = -100
  ...
  func makeLoadPlan(for source: VideoAssetSourceDescriptor) -> VideoAssetLoadPlan? {
    ...
    guard canCache(source: source) else {
      print("[nitro-video] Provided source with uri: \(source.url.absoluteString) cannot be cached. Caching will be disabled")
      return nil
    }
    ...
    guard let variantFilePath = VideoAsset.pathForUrl(url: source.url, fileExtension: fileExtension, variantKey: variantKey),
      ...
      print("[nitro-video] Failed to create a cache file path for the provided source with uri: \(source.url.absoluteString)")
      return nil
    }
    ...
    guard let urlWithCustomScheme = source.url.withScheme(VideoCacheManager.expoVideoCacheScheme) else {
      print("[nitro-video] CachingPlayerItem error: Urls without a scheme are not supported, the resource won't be cached")
      return nil
    }
