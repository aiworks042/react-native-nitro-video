package com.margelo.nitro.nitrovideo.delegates
 ...
 */
class IgnoreSameSet<T : Any?>(
    private var value: T,
    val propertyMapper: ((T) -> T) = { v -> v },
    val didSet: ((new: T, old: T) -> Unit)? = null
) {
    ...
    operator fun setValue(thisRef: Any?, property: KProperty<*>, value: T) {
        if (this.value?.equals(propertyMapper(value)) == true) return
